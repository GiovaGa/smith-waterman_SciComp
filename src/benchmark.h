...
void benchmark(struct sw_implementation* implementation, int n_implementations, struct sequence_t A, struct sequence_t B, struct scores_t *scores_param)
{
	...
	for (int i = 0; i < n_implementations; i++)
	{
		...
		score = implementation[i].function(A.length, B.length, A.data, B.data, scores_param->match, scores_param->gap_opening, scores_param->gap_extension, scores_param->mismatch, H);
